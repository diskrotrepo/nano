...
- **Lyrics** (the actual words to sing): conditioned as a **phoneme-ID sequence**, NOT through CLAP. `text_to_phoneme_ids` (g2p_en) converts the lyric string to ARPABET phonemes; a trainable `LyricEncoder` ([model/lyric_encoder.py](model/lyric_encoder.py)) — a small bidirectional transformer living *inside* `NanoAudioGPT` — encodes them into a sequence, and each transformer block has a **separate lyric cross-attention** over it. The model learns its own (near-monotonic) alignment, so no inference-time timestamps/duration model are needed. This is what makes the model sing intelligible words; a single pooled CLAP "vibe" vector structurally cannot.
  ...
  **Markers in the lyric stream** — the phoneme stream also carries non-word **marker tokens** the decoder cross-attends to (frozen in `PHONEME_VOCAB`): *song-structure* markers (`<verse>`, `<chorus>`, …, from the allin1 analyzer), *vocal-gender* markers (`<male>`/`<female>`/`<unknown_gender>`, from an F0 heuristic on the Demucs vocal stem — see `transcribe_lyrics.estimate_vocal_gender`), and *tempo* markers (`<tempo_0>`…`<tempo_6>`/`<unknown_tempo>`, the allin1 per-song `bpm` bucketed by `TEMPO_BPM_EDGES`). Tempo rides the stream because chroma is octave-invariant pitch and structurally can't carry beat rate; the `bpm` field was already computed by the structure pass and previously discarded by the dataset loader, so it's a near-free axis. Every stream opens with a compact dense header `BOS <gender> <tempo> <section>` (with `<unknown_gender>`/`<unknown_tempo>`/`<no_section>` fallbacks so no row is fully padded); structure boundaries inside a crop are injected inline (gender and tempo are per-song, prefix-only). The train-time injector (`TokenDataset._get_segment_lyric_ids`) and the inference parser (`text_with_markers_to_phoneme_ids`, which accepts `[female] [120bpm] [chorus] …` brackets — bpm as a numeric, `tempo:NNN`, or `slow`/`medium`/`fast` form) MUST build byte-identical streams — `tests/test_structure_markers.py` is the guard. Adding/removing any marker changes `PHONEME_VOCAB_SIZE` and is checkpoint-incompatible.
...
Each stream drops independently for classifier-free guidance (10% each during training; melody drops to its learned null); at inference `cfg_scale` guides them jointly, or an optional `lyric_cfg_scale` / `melody_cfg_scale` pushes that axis harder via composed guidance. The `LyricEncoder` and `MelodyEncoder` are submodules of the model, so they save/restore in the `model` state_dict (no sidecar key like CLAP's `text_proj`). Checkpoints from before the lyric encoder (v7) are incompatible; v8 (`ckpt_subdir="v8_sing"`) ships the lyric encoder, the melody encoder, **and** the FIM control ids in one fresh start — adding the melody encoder and the FIM sentinels are each checkpoint-incompatible, and melody requires the pack to carry the chroma sidecar.
...
9. **Train**: packed shards (+ chroma sidecar) + tags.json + lyrics/ + structure/ → TokenDataset (mmap-backed random 30s crops) → delayed sequence → cross-entropy loss per codebook
10. **Inference**: checkpoint → NanoAudioGPT → autoregressive generation with KV cache → DAC decode → MP3 via ffmpeg (the `/cover` path additionally feeds the uploaded hum's chromagram)
...
| nano-tokens | .pt token files, packed/ shards (incl. `.mel.bin`), tags.json, lyrics/, structure/ |
...
- **Add more training data**: Add MP3s to corpus, re-run prepare + tokenize + melody + pack + auto-tag + transcribe + structure, then re-train (the `add-songs` skill walks this end-to-end). More of the same kind of data is the goal — don't curate for variety.
