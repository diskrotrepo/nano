...
def gen_one(eng, setting, prompt_key, clip_idx, tag):
    ...
    print(f"[{rec['gen_sec']:5.1f}s] {setting['id']:28s} {prompt_key:7s} "
...
def main():
    ...
    if args.stage == 1:
        ...
        for s in settings:
            for pk in C.PROMPTS:
                recs.append(gen_one(eng, s, pk, 0, tag))
    elif args.stage == 2:
        ...
        for s in settings:
            for pk in C.PROMPTS:
                for clip in (1, 2):
                    recs.append(gen_one(eng, s, pk, clip, tag))
